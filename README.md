...
* `rdi-installer-<version>.<arch>.img` (USB stick)
...
This binary runs completley from a RAM disk in memory.


###  rdi-installer-&lt;version&gt;.&lt;arch&gt;.img
...
This system also runs completley from a RAM disk in memory.
...
| rdii.autoinstall.confirm_infos | true/false/yes/no/1/0 | Only takes effect if `rdii.autoinstall` is `true`. Default `true`: informational popups are always shown. If `false`, the popup is automatically dismissed after `rdii.autoinstall.popup_timeout` seconds instead of waiting for a keypress; if `rdii.autoinstall.popup_timeout` is `0` (the default), a built-in 30 second timeout is used instead, since info popups are status notices rather than decisions |
